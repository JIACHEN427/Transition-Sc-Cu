...
subroutine advection
  ...
  use modglobal,  only : lmoist, nsv,nsc, iadv_mom,iadv_tke,iadv_thl,iadv_qt,iadv_sv,iadv_sc, &
                         iadv_cd2,iadv_5th,iadv_52,iadv_cd6,iadv_62,iadv_kappa,iadv_upw,iadv_hybrid,iadv_hybrid_f,iadv_null,leq
  use modfields,  only : u0,up,v0,vp,w0,wp,e120,e12p,thl0,thlp,qt0,qtp,sv0,svp,sc0,scp
  ...
  do n=1,nsv
   ...
   do n=1,nsc
     select case(iadv_sc(n))
     case(iadv_cd2)
       call advecc_2nd(sc0(:,:,:,n),scp(:,:,:,n))
     case(iadv_5th)  
     if (.not. leq) stop "advec_5th does not support a non-uniform vertical grid."
       call advecc_5th(sc0(:,:,:,n),scp(:,:,:,n))
     case(iadv_52)
       call advecc_52(sc0(:,:,:,n),scp(:,:,:,n))
     case(iadv_cd6)
       if (.not. leq) stop "advec_6th does not support a non-uniform vertical grid."
      call advecc_6th(sc0(:,:,:,n),scp(:,:,:,n))
     case(iadv_62)
      call advecc_62(sc0(:,:,:,n),scp(:,:,:,n))
     case(iadv_kappa)
       if (.not. leq) stop "advec_kappa does not support a non-uniform vertical grid."
       call advecc_kappa(sc0(:,:,:,n),scp(:,:,:,n))
     case(iadv_upw)
       if (.not. leq) stop "advec_upw does not support a non-uniform vertical grid."
       call advecc_upw(sc0(:,:,:,n),scp(:,:,:,n))
     case(iadv_hybrid)
       if (.not. leq) stop "advec_hybrid does not support a non-uniform vertical grid."
       call advecc_hybrid(sc0(:,:,:,n),scp(:,:,:,n))
     case(iadv_hybrid_f)
       if (.not. leq) stop "advec_hybrid_f does not support a non-uniform vertical grid."
       call advecc_hybrid_f(sc0(:,:,:,n),scp(:,:,:,n))
     case(iadv_null)
        ! null advection scheme - do nothing
     case default
       stop "Unknown advection scheme "
