...
!> Sets lateral periodic boundary conditions for the scalars
 subroutine cyclich
  ...
  use modglobal, only : i1,ih,j1,jh,k1,nsv,nsc
  use modfields, only : thl0,thlm,qt0,qtm,sv0,svm,sc0,scm
  ...
  enddo
  do n=1,nsc
    call excjs( sc0(:,:,:,n)   , 2,i1,2,j1,1,k1,ih,jh)
    !call excjs( svm(:,:,:,n)   , 2,i1,2,j1,1,k1,ih,jh)
  enddo
...
!! \endlatexonly
 subroutine grwdamp
  use modglobal, only : i1,i2,j1,j2,kmax,cu,cv,lcoriol,igrw_damp,geodamptime,nsv,nsc,rdt,unudge,dzf
  use modfields, only : up,vp,wp,thlp,qtp,u0,v0,w0,thl0,qt0,sv0,sc0,ug,vg &
                        ,thl0av,qt0av,sv0av,sc0av,u0av,v0av,scprof
  implicit none
  ...
  do n=1,nsc
    sc0(2:i1,2:j1,kmax,n) = sc0av(kmax,n)
  end do
...
!> Sets top boundary conditions for scalars
  ...
  use modglobal, only : i2,j2,kmax,k1,nsv,nsc,dtheta,dqt,dsv,dsc,dzh,zh,rdt &
                       ,lrelax
  use modfields, only : thl0,thlm,qt0,qtm,sv0,svm,sc0,scm &
                       ,thl0av,qt0av,sv0av,sc0av,scprof
  use modtimestat, only : zi
  implicit none
  integer :: n,i,j,k,taubl=400,tauft=3200
...
! **  Top conditions :
  ...
  enddo
  do n=1,nsc
    dsc(n) = sum((sc0av(kmax-kav+1:kmax,n)-sc0av(kmax-kav:kmax-1,n))/ &
               dzh(kmax-kav:kmax-1))/kav
  enddo
  ...
  enddo
  do n=1,nsc
    sc0(:,:,k1,n) = sc0(:,:,kmax,n) + dsc(n)*dzh(k1)
    scm(:,:,k1,n) = scm(:,:,kmax,n) + dsc(n)*dzh(k1)
  enddo


if (nsc>10 .and. lrelax) then ! Relax the passive scalar to its orginal value (test) JCLu
! ref. DAVINI(2017)
! Scalar BL
  do k=1,kmax
     do j=1,j2
        do i=1,i2
           sc0(i,j,k,1) = rdt/taubl*(zh(k)/1000-scm(i,j,k,1)) + scm(i,j,k,1) 
         end do
     end do
  end do
! Scalar FT
...
write(*,*) 'relaxing scalars'
  do k=1,kmax
     do j=1,j2
        do i=1,i2
           sc0(i,j,k,2) = rdt/tauft*(scprof(k,2)-scm(i,j,k,2)) + scm(i,j,k,2) 
        end do
      end do
    end do
