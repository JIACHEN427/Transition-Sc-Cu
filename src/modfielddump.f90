...
module modfielddump
  ...
  use modglobal, only : longint, nsv, nsc
...
!       call ncinfo(ncname( 7,:),'qr','Rain water mixing ratio','1e-5kg/kg','tttt')
      ...
      do m=1,nsc
        write (csvname(1:3),'(i3.3)') m
        call ncinfo(ncname(9+m,:),'sc'//csvname,'Sc '//csvname//' specific concentration','(kg/kg)','tttt')
      end do
...
!> Do fielddump. Collect data to truncated (2 byte) integers, and write them to file
  subroutine fielddump
    use modfields, only : u0,v0,w0,thl0,qt0,ql0,sv0,sc0,thv0h,thvh
    ...
    if (lnetcdf) vars(:,:,:,8:nvar-nsc) = sv0(2:i1:ncoarse,2:j1:ncoarse,klow:khigh,:)
    if (lnetcdf) vars(:,:,:,nvar-nsc+1:nvar) = sc0(2:i1:ncoarse,2:j1:ncoarse,klow:khigh,:)
    if(lnetcdf) then
