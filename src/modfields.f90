...
save
  ...
  real, allocatable :: scm(:,:,:,:)   !<  scalar sc(n) at time step t-1
  real, allocatable :: sc0(:,:,:,:)   !<  scalar sc(n) at time step t
  real, allocatable :: scp(:,:,:,:)   !<  tendency of sc(n)
  ...
  real, allocatable :: sc0av(:,:)                  !<   slab average of sc(n)
  real, allocatable :: scprof(:,:)                 !<   initial sc(n)-profile
...
subroutine initfields
    ...
    allocate(scm(2-ih:i1+ih,2-jh:j1+jh,k1,nsc))
    allocate(sc0(2-ih:i1+ih,2-jh:j1+jh,k1,nsc))
    allocate(scp(2-ih:i1+ih,2-jh:j1+jh,k1,nsc))
    ...
    allocate(sc0av(k1,nsc))
    allocate(scprof(k1,nsc))
    allocate(thlpcar(k1))
    ...
    qt0av=0.;ql0av=0.;thl0av=0.;u0av=0.;v0av=0.;sv0av=0.;sc0av=0.
    thlprof=0.;qtprof=0.;uprof=0.;vprof=0.;e12prof=0.;svprof=0.;scprof=0
...
!> Deallocate the fields
  ...
  implicit none
    ...
    deallocate(thlprof,qtprof,uprof,vprof,e12prof,sv0av,svprof,sc0av,scprof)
    deallocate(thlpcar)
