...
!-----------------------------------------------------------------|
  ...
  use modfields, only : sv0,sc0,up,vp,wp,thv0h,dpdxl,dpdyl,thvh
...
!-----------------------------------------------------------------|
  ...
  use modglobal, only : i1,j1,kmax,dzh,nsv,nsc,lmomsubs
  use modfields, only : up,vp,thlp,qtp,svp,scp,&
                        whls, u0av,v0av,thl0,qt0,sv0,sc0,u0,v0,&
  ...
  real subs_thl,subs_qt,subs_sv,subs_sc,subs_u,subs_v
  ...
  do j=2,j1
    do i=2,i1
      ...
      if (whls(2).lt.0) then !neglect effect of mean ascending on tendencies at the lowest full level
        ...
        enddo
        do n=1,nsc
          subs_sc =  0.5*whls(2)  *(sc0(i,j,2,n)-sc0(i,j,1,n)  )/dzh(2)
          scp(i,j,1,n) = scp(i,j,1,n)-subs_sc
        enddo
      endif
    ...
    do j=2,j1
      do i=2,i1
        if (whls(kp).lt.0) then   !downwind scheme for subsidence
          ...
          enddo
          do n=1,nsc
            subs_sc   = whls(kp)  *(sc0(i,j,kp,n) - sc0(i,j,k,n))/dzh(kp)
            scp(i,j,k,n) = scp(i,j,k,n)-subs_sc
          enddo
          ...
          do n=1,nsv
            ...
            svp(i,j,k,n) = svp(i,j,k,n)-subs_sv
          enddo
! Passive scalar, calculate the subsidence and tendency.
          do n=1,nsc
            subs_sc   = whls(k) * (sc0(i,j,k,n) - sc0(i,j,km,n))/dzh(k)
            scp(i,j,k,n) = scp(i,j,k,n)-subs_sc
          enddo
        endif
