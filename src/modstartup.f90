...
contains
  subroutine startup
    ...
    use modglobal,         only : initglobal,iexpnr, ltotruntime, runtime, dtmax, dtav_glob,timeav_glob,&
                                  ...
                                  nc_input,lrelax,taubl,tauft,&
                                  nsv,nsc,itot,jtot,kmax,xsize,ysize,xlat,xlon,xday,xtime,&
                                  lmoist,lcoriol,lpressgrad,igrw_damp,geodamptime,lmomsubs,cu, cv,ifnamopt,fname_options,llsadv,&
                                  ibas_prf,lambda_crit,iadv_mom,iadv_tke,iadv_thl,iadv_qt,iadv_sv,iadv_sc,courant,peclet,ladaptive,author,lnoclouds,lrigidlid,unudge
    use modforces,         only : lforce_user
    use modsurfdata,       only : z0,ustin,wtsurf,wqsurf,wsvsurf,wscsurf,ps,thls,isurf
    ...
    use modtimedepsc,      only : inittimedepsc,ltimedepsc
    ...
    namelist/RUN/ &
        ...
        trestart,irandom,randthl,randqt,krand,nsv,nsc,courant,peclet,ladaptive,author,&
        krandumin, krandumax, randu,&
        nprocx,nprocy,nc_input,lrelax,taubl,tauft
    namelist/DOMAIN/ &
        itot,jtot,kmax,&
        xsize,ysize,&
    ...
    namelist/PHYSICS/ &
        ...
        z0,ustin,wtsurf,wqsurf,wsvsurf,wscsurf,ps,thls,lmoist,isurf,chi_half,&
        lcoriol,lpressgrad,igrw_damp,geodamptime,lmomsubs,ltimedep,ltimedepsv,irad,timerad,iradiation,rad_ls,rad_longw,rad_shortw,rad_smoke,useMcICA,ltimedepsc,&
        rka,dlwtop,dlwbot,sw0,gc,reff,isvsmoke,lforce_user,lcloudshading,lrigidlid,unudge
    namelist/DYNAMICS/ &
        llsadv,  lqlnr, lambda_crit, cu, cv, ibas_prf, iadv_mom, iadv_tke, iadv_thl, iadv_qt,&
 iadv_sv,iadv_sc ,lnoclouds
  ...
  !broadcast namelists
    ...
    call MPI_BCAST(nsc        ,1,MPI_INTEGER,0,MPI_COMM_WORLD,mpierr)
    ...
    call MPI_BCAST(lrelax     ,1,MPI_LOGICAL,0,MPI_COMM_WORLD,mpierr)
    call MPI_BCAST(taubl      ,1,MPI_INTEGER,0,MPI_COMM_WORLD,mpierr)
    call MPI_BCAST(tauft      ,1,MPI_INTEGER,0,MPI_COMM_WORLD,mpierr)
    ...
    call MPI_BCAST(wscsurf(1:nsc),nsc,MY_REAL   ,0,MPI_COMM_WORLD,mpierr)
    ...
    call MPI_BCAST(ltimedepsc ,1,MPI_LOGICAL,0,MPI_COMM_WORLD,mpierr)
    ...
    call MPI_BCAST(iadv_sc(1:nsc) ,nsc,MPI_INTEGER,0,MPI_COMM_WORLD,mpierr)
  ...
  subroutine readinitfiles
    use modfields,         only : u0,v0,w0,um,vm,wm,thlm,thl0,thl0h,qtm,qt0,qt0h,&
                                  ql0,ql0h,thv0h,sv0,svm,sc0,scm,e12m,e120,&
                                  dudxls,dudyls,dvdxls,dvdyls,dthldxls,dthldyls,&
                                  dqtdxls,dqtdyls,dqtdtls,dpdxl,dpdyl,&
                                  wfls,whls,ug,vg,uprof,vprof,thlprof, qtprof,e12prof,svprof,scprof,&
                                  v0av,u0av,qt0av,ql0av,thl0av,sv0av,sc0av,exnf,exnh,presf,presh,rhof,&
                                  thlpcar,thvh,thvf
    use modglobal,         only : i1,i2,ih,j1,j2,jh,kmax,k1,dtmax,idtmax,dt,rdt,runtime,timeleft,tres,&
                                  rtimee,timee,ntrun,btime,dt_lim,nsv,nsc,&
    ...
    use modsurfdata,       only : wsvsurf,wscsurf,&
                                  thls,tskin,tskinm,tsoil,tsoilm,phiw,phiwm,Wl,Wlm,thvs,qts,isurf,svs,scs,obl,oblav,&
    ...
    integer :: dimIDNt,dimIDnsv,dimIDnsc
    integer :: nsv_in,nsc_in         ! number of passive scalars present in ncdf input file. if nsv_in < nsv, profiles are assumed to be zero
...
!> Read the initial profile of the passive scalar.
       scprof = 0.
       if(myid==0)then
         if (nsc>0) then
               scprof(:,:) = 0.   !cstep assumes zero values
            if (nc_input) then  !cstep
                sts          = nf90_inq_dimid(ncid,"nsc",dimIDnsc)
                if (sts.ne.nf90_noerr) then
                   write (6,*) 'warning: nsc not present!'
                else
                   sts          = nf90_inquire_dimension(ncid, dimIDnsc, tmpName,nsc_in)
                endif
                if (nsc_in.eq.nsc) then
                   sts          = nf90_inq_varid(ncid,"sc",varID)
                   sts          = nf90_get_var(ncid, varID, scprof(1:kmax,1:nsc))
                else
                  write (6,*) 'scalars are initialized to zero'
               end if
            else
               open (ifinput,file='sc.inp.'//cexpnr)
               read (ifinput,'(a80)') chmess
               read (ifinput,'(a80)') chmess
               do k=1,kmax
                 read (ifinput,*) &
        ...
        call MPI_BCAST(wscsurf,nsc   ,MY_REAL   ,0,comm3d,mpierr)
        call MPI_BCAST(scprof ,k1*nsc,MY_REAL   ,0,comm3d,mpierr)
        do k=1,kmax
          do j=1,j2
            do i=1,i2
              do n=1,nsc
                sc0(i,j,k,n) = scprof(k,n)
                scm(i,j,k,n) = scprof(k,n)
              end do
            end do
          end do
        end do
...
!      end do
      ...
      do n=1,nsc
        call slabsum(sc0av(1,n),1,k1,sc0(1,1,1,n),2-ih,i1+ih,2-jh,j1+jh,1,k1,2,i1,2,j1,1,k1)
      end do
...
!******include rho if rho = rho(z) /= 1.0 ***********
  ...
  subroutine readrestartfiles
    ...
    use modsurfdata, only : ustar,thlflux,qtflux,svflux,scflux,dthldz,dqtdz,ps,thls,qts,thvs,oblav,&
    ...
    use modfields,  only : u0,v0,w0,thl0,qt0,ql0,ql0h,e120,dthvdz,presf,presh,sv0,sc0,tmp0,esl,qvsl,qvsi
    use modglobal,  only : i1,i2,ih,j1,j2,jh,k1,dtheta,dqt,dsv,dsc,startfile,timee,&
                           tres,ifinput,nsv,nsc,dt
...
!       v0 = v0-cv
    ...
    if (nsc>0) then
       ...
       write(6,*) 'loading ',name
       open(unit=ifinput,file=name,form='unformatted')
       read(ifinput) ((((sc0(i,j,k,n),i=2-ih,i1+ih),j=2-jh,j1+jh),k=1,k1),n=1,nsc)
       read(ifinput) (((scflux(i,j,n),i=1,i2),j=1,j2),n=1,nsc)
       read(ifinput) (dsc(n),n=1,nsc)
       read(ifinput)  timee
       close(ifinput)
     end if
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
  subroutine writerestartfiles
    ...
    use modraddata, only: iradiation, useMcICA
    use modfields, only : u0,v0,w0,thl0,qt0,ql0,ql0h,e120,dthvdz,presf,presh,sv0,sc0,tmp0,esl,qvsl,qvsi
    use modglobal, only : i1,i2,ih,j1,j2,jh,k1,dsv,dsc,trestart,itrestart,tnextrestart,dt_lim,rtimee,timee,tres,cexpnr,&
                          rtimee,rk3step,ifoutput,nsv,nsc,timeleft,dtheta,dqt,dt
    ...
    if ((timee>=tnextrestart .and. trestart > 0) .or. (timeleft==0 .and. trestart >= 0)) then
      ...
      if (nsc>0) then
         name(5:5) = 'c'
        open  (ifoutput,file=name,form='unformatted')
        write(ifoutput) ((((sc0(i,j,k,n),i=2-ih,i1+ih),j=2-jh,j1+jh),k=1,k1),n=1,nsc)
        write(ifoutput) (((scflux(i,j,n),i=1,i2),j=1,j2),n=1,nsc)
        write(ifoutput) (dsc(n),n=1,nsc)
        write(ifoutput)  timee
        close (ifoutput)
      end if
      if (isurf == 1) then
