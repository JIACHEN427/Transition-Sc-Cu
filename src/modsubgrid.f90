...
! Thijs Heus, Chiel van Heerwaarden, 15 June 2007
    ...
    use modglobal, only : nsv,nsc, lmoist
    use modfields, only : up,vp,wp,e12p,thl0,thlp,qt0,qtp,sv0,svp,sc0,scp
    use modsurfdata,only : thlflux,qtflux,svflux,scflux
    implicit none
    integer n
    ...
    do n=1,nsc
      call diffc(sc0(:,:,:,n),scp(:,:,:,n),scflux(:,:,n))
    end do
