...
!> Reads the namelists and initialises the soil.
  subroutine initsurface
    ...
    use modglobal,  only : i1, j1, i2, j2, itot, jtot, nsv,nsc, ifnamopt, fname_options, ifinput, cexpnr
 ...
 character(len=1500) :: readbuffer
    namelist/NAMSURFACE/ & !< Soil related variables
      ...
      z0, thls, ps, ustin, wtsurf, wqsurf, wsvsurf,wscsurf, &
    ...
    call MPI_BCAST(wscsurf(1:nsc),nsc,MY_REAL   ,0,comm3d,mpierr)
    ...
    if(lhetero) then
      ...
      allocate(wsc_patch(100,xpatches,ypatches))
      ...
      defined_landtypes = 0
!\todo Use prescried surface value for the passive scalars?
...
!> Calculates the interaction with the soil, the surface temperature and humidity, and finally the surface fluxes.
  subroutine surface
    use modglobal,  only : i1,i2,j1,j2,fkar,zf,cu,cv,nsv,nsc,ijtot,rd,rv
...
!> Calculates surface resistance, temperature and moisture using the Land Surface Model
  subroutine do_lsm
    ...
    use modglobal, only : pref0,boltz,cp,rd,rhow,rlv,i1,j1,rdt,ijtot,rk3step,nsv,nsc,xtime,rtimee,xday,xlat,xlon
