...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
  subroutine inittimedep
    ...
    use modtimedepsc, only : inittimedepsc
    ...
    integer :: dimIDkls,dimIDkflux,dimIDnsv,dimIDnsc
 ...
 !read(ifinput,*, iostat = ierr) timeflux(t), wtsurft(t), wqsurft(t),thlst(t),qtst(t),pst(t)
           ...
           t    = 0
           ierr = 0
           do while (timeflux(t) <= runtime) ! add a equal sign here.
             t=t+1
             read(ifinput,*, iostat = ierr) timeflux(t), wtsurft(t), wqsurft(t),thlst(t),qtst(t),pst(t)
...
!-----------------------------------------------------------------|
    ...
    use modtimedepsc, only : timedepsc
    implicit none
...
!cstep: not necessary to provide qts in ls_flux file qts    = qtst(t)    + fac * ( qtst(t+1)    - qtst(t)     )
  ...
  subroutine exittimedep
    ...
    use modtimedepsc, only : exittimedepsc
    implicit none
    if (.not. ltimedep) return
